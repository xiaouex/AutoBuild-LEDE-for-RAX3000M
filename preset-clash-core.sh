...
CLASH_TUN_URL=$(curl -fsSL https://api.github.com/repos/vernesong/OpenClash/contents/master/premium\?ref\=core | grep download_url | grep "arm64" | awk -F '"' '{print $4}' | grep "v3" )
